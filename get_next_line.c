...
/*   Updated: 2025/02/19 17:13:08 by dacrespo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
//------------READ_LINE----------------
/*
Se llama a read_line para leer contenido adicional del archivo y
actualizar backup.
Lee datos del archivo en trozos de tamaño BUFFER_SIZE hasta encontrar un salto
de línea (\n) o hasta que no haya más datos que leer.
Lee el archivo fd y guarda la información leída en buffer.
El bucle se ejecuta hasta que encuentra un carácter de nueva línea '\n'en el
búfer existente backup.
...
char	*read_line(int fd, char *backup)
{
	...
	while (!ft_strchr(backup, '\n'))
	{
		...
		if (bytes_read == 0)
			break ;
		backup = ft_join_free(backup, buffer);
	}
	free(buffer);
	return (backup);
}
...
/*------------------------------EXTRACT_LINE----------------------------------
Se llama a extract_line para obtener la primera línea completa de backup.
Esta función toma backup y extrae la línea hasta el primer carácter de
nueva línea '\n'.
El resultado es una nueva cadena que contiene la línea extraída.
Si backup está vacío, devuelve NULL.

Preparar para la próxima llamada: Llama a next_line para actualizar backup
con cualquier contenido que quede después de la línea extraída
Propósito: Actualiza el backup para contener solo el contenido después de la
línea extraída, preparando backup para la próxima llamada a get_next_line.

Esta función crea una nueva cadena new_backup que contiene el contenido
 de backup después de la primera línea.
Libera la memoria de backup y devuelve la nueva cadena.
Si backup está vacío, devuelve NULL.


PSEUDOCÓDIGO:
Función extract_line(backup)
	Declarar variable line
	Declarar variable i e inicializar en 0

	Si backup[i] es el carácter nulo ('\0') entonces
		Retornar NULL
	Fin Si

	Mientras backup[i] sea diferente de '\n' y diferente de '\0' hacer
		Incrementar i
	Fin Mientras

	Si backup[i] es igual a '\n' entonces
		Incrementar i
	Fin Si

	Asignar a line el resultado de llamar a ft_substr(backup, 0, i)

	Si line es NULL entonces
		Retornar NULL
	Fin Si
...
Si hay un error en los parámetros de entrada o en la lectura del archivo,
libera la memoria y devuelve NULL.
Utiliza las funciones anteriores para leer la línea, extraerla y actualizar el
backup para la siguiente llamada.
